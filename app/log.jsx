...
const SUGGESTIONS = [
  "2 roti + dal makhani", "oats with banana", "idli sambar",
  "chicken sandwich", "poha and chai", "rajma chawal",
];
    ...
    <View style={s.root}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <ScrollView style={s.scroll} contentContainerStyle={s.content}
          ...
          <View style={s.mealRow}>
            {MEAL_TYPES.map((m) => (
              <TouchableOpacity key={m.label}
                ...
                onPress={() => setMealType(m.label)} activeOpacity={0.75}>
                <Text style={s.mealEmoji}>{m.emoji}</Text>
                <Text style={[s.mealTxt, mealType === m.label && s.mealTxtOn]}>{m.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {/* Suggestions */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: rs(20) }}>
            {SUGGESTIONS.map((sg) => (
              <TouchableOpacity key={sg} style={s.sugg} onPress={() => setInput(sg)}>
          ...
          <TouchableOpacity
            ...
            onPress={analyse} disabled={!input.trim() || loading} activeOpacity={0.85}>
            {loading
          ...
          {result && (
            <View style={[s.resultCard, SHADOW.md]}>
              ...
              {result.items.map((item, i) => (
                <View key={i} style={[s.itemRow, i < result.items.length - 1 && s.itemBorder]}>
              ...
              <View style={s.macroStrip}>
                {[
                  ...
                  <View key={m.l} style={[s.macroItem, i < arr.length - 1 && s.macroDivide]}>
              ...
              {result.tip && (
                <View style={s.tipRow}>
                  <View style={s.tipBadge}><Text style={s.tipBadgeTxt}>AI Tip</Text></View>
              ...
              <TouchableOpacity
                ...
                onPress={addToDiary} disabled={added} activeOpacity={0.85}>
                <Text style={s.addTxt}>{added ? "✓  Added!" : `Add to ${mealType}`}</Text>
              </TouchableOpacity>
            </View>
          )}
...
const s = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: rs(26) },
  ...
  secLabel: { fontSize: rf(11), fontWeight: "800", color: COLORS.muted, textTransform: "uppercase", letterSpacing: 1, marginBottom: rs(10) },

  mealRow: { flexDirection: "row", gap: rs(8), marginBottom: rs(24) },
  mealChip: { flex: 1, alignItems: "center", paddingVertical: rs(12), backgroundColor: COLORS.card, borderRadius: rs(18), borderWidth: rs(2), borderColor: "transparent", ...SHADOW.sm },
  ...
  inputWrap: { backgroundColor: COLORS.card, borderRadius: rs(20), padding: rs(18), marginBottom: rs(12) },
  input: { fontSize: rf(15), color: COLORS.dark, minHeight: rs(88), textAlignVertical: "top", lineHeight: rf(22) },

  sugg: { backgroundColor: COLORS.greenLight, borderRadius: rs(99), paddingHorizontal: rs(14), paddingVertical: rs(8), marginRight: rs(8) },
  suggTxt: { fontSize: rf(12), color: COLORS.green, fontWeight: "600" },

  analyseBtn: { backgroundColor: COLORS.dark, borderRadius: rs(18), paddingVertical: rs(18), alignItems: "center", marginBottom: rs(14), ...SHADOW.md },
  analyseBtnOff: { backgroundColor: COLORS.border, shadowOpacity: 0, elevation: 0 },
  analyseTxt: { color: "#fff", fontSize: rf(16), fontWeight: "700" },

  errorCard: { backgroundColor: "#FEF2F2", borderRadius: rs(14), padding: rs(14), marginBottom: rs(14), borderLeftWidth: rs(4), borderLeftColor: COLORS.red },
  errorTxt: { color: "#b91c1c", fontSize: rf(14), fontWeight: "500" },

  resultCard: { backgroundColor: COLORS.card, borderRadius: rs(24), overflow: "hidden", marginBottom: rs(16) },

  itemRow: { flexDirection: "row", alignItems: "center", paddingHorizontal: rs(18), paddingVertical: rs(14) },
  ...
  macroStrip: { flexDirection: "row", backgroundColor: COLORS.cardAlt, paddingVertical: rs(16) },
  ...
  macroLbl: { fontSize: rf(10), color: COLORS.muted, marginTop: rs(2), fontWeight: "600" },

  tipRow: { flexDirection: "row", alignItems: "flex-start", gap: rs(10), padding: rs(16), borderTopWidth: 1, borderTopColor: COLORS.border },
  tipBadge: { backgroundColor: COLORS.greenLight, borderRadius: rs(99), paddingHorizontal: rs(10), paddingVertical: rs(4) },
  tipBadgeTxt: { fontSize: rf(10), fontWeight: "800", color: COLORS.green },
  tipTxt: { flex: 1, fontSize: rf(13), color: COLORS.mid, lineHeight: rf(19), fontWeight: "500" },

  addBtn: { backgroundColor: COLORS.dark, margin: rs(14), borderRadius: rs(16), paddingVertical: rs(16), alignItems: "center" },
